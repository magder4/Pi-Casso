...
# setting up communication with the pi
HOST = "10.33.142.80"  # The server's hostname or IP address    
...
class Frontend:
    ...
    def _handle_connect_response(self, error):
        ...
        if not error:
            ...
            # Sets the GAZE_IN_SCREEN data stream rate to 50Hz
...
class TrackingWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # create a qpainter with the marker pixmap  
        painter = QtGui.QPainter(qt_marker_image)

        painter.drawLine(int(QtWidgets.QApplication.instance().primaryScreen().geometry().width()/3),0 ,
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().width())/3), 
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().height())))
        
        painter.drawLine(int((QtWidgets.QApplication.instance().primaryScreen().geometry().width()/3)*2),0 ,
                        (int((QtWidgets.QApplication.instance().primaryScreen().geometry().width())/3)*2), 
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().height())))

        painter.drawLine(0, int(QtWidgets.QApplication.instance().primaryScreen().geometry().height()/3),
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().width())), 
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().height())/3))
        
        painter.drawLine(0, int(QtWidgets.QApplication.instance().primaryScreen().geometry().height()/3)*2,
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().width())), 
                        (int(QtWidgets.QApplication.instance().primaryScreen().geometry().height())/3)*2)
