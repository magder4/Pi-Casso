...
#Setup pi communication
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect(('10.33.142.80', 3434))
print("Connected")
...
class Frontend:
    ...
    def _handle_event_stream(self, event_type, _timestamp, *_args):
        ...
        if event_type == Events.BLINK:
            if float(*_args) > BlinkTime: #make sure blink is long enough
    ...
    def _handle_connect_response(self, error):
        ...
        if not error:
            ...
            # Sets the GAZE_IN_SCREEN data stream rate to 60Hz
